"""
OpenRouter OCR Engine Implementation
======================================
Uses OpenRouter's OpenAI-compatible API (https://openrouter.ai/api/v1) to
perform OCR + layout analysis through a vision-capable model routed through
OpenRouter (Claude, GPT, Gemini, Llama, etc.) in a single request per page —
another alternative to gemini_engine.py's direct-to-Gemini approach, for
anyone who already has an OpenRouter account/credits.

Why you might pick this over the default "gemini" engine:
- Reuse an existing OpenRouter credit balance instead of a separate Google
  AI Studio key
- Swap the underlying model by model slug (OPENROUTER_MODEL) without
  touching code — any vision-capable model on OpenRouter works, not just
  Gemini
- Unlike Poe, OpenRouter bills in ordinary per-token credits (no separate
  points system), and many providers' models are mirrored there under a
  stable `provider/model-name` slug (e.g. "anthropic/claude-sonnet-4.5")

Model selection:
OpenRouter has no platform-wide default model either — every request must
name one, and which models are worth using depends on your account/budget.
Set the OPENROUTER_MODEL environment variable (or ocr.openrouter_model_name
in config.yaml) to a vision-capable model slug, e.g.
"anthropic/claude-sonnet-4.5" — confirm the exact slug at
https://openrouter.ai/models (filter for image input support) first.

User-provided credentials (Settings page):
Same contract as PoeOCREngine — the app lets the logged-in user enter their
own OPENROUTER_API_KEY / model slug at runtime via the Settings page
(Api/static/settings.html → POST /api/settings), stored in Redis under
"settings:openrouter_api_key" / "settings:openrouter_model". worker.py
calls refresh_runtime_settings() once per job, which re-resolves
self.api_key / self.model_name from Redis if present, else falls back to
the environment/config.yaml value this engine was constructed with.
Because credentials may only be supplied AFTER the worker thread has
already started, load() does NOT fail if they are missing — only the
first actual OCR call does, with an actionable error.

Rate-limiting:
Unlike Poe, OpenRouter has no single flat per-account RPM ceiling — limits
are mostly inherited from the upstream provider of whichever model you
pick, and vary by plan. The one documented exception is free-tier
(":free"-suffixed) models, capped at 20 requests/minute and 50-1000
requests/day depending on purchased credits. The engine self-throttles to
stay within the configured RPM (ocr.rpm_limit), but does NOT clamp it to
a guessed ceiling the way PoeOCREngine clamps to 500 — tune rpm_limit in
config.yaml yourself to match whichever model you select (20 or lower for
a free model; higher is usually fine for a paid one, subject to your
account standing).

Persistent per-page caching:
Same contract as GeminiOCREngine/PoeOCREngine — the worker can pre-populate
this engine's in-memory cache with a previously saved page result via
`prime_page_cache_from_dict`, and read back the just-computed result via
`export_last_page_result`, so a paused/resumed job doesn't re-spend
credits on pages already OCR'd.
...
# ── OCR prompt — same schema gemini_engine.py / poe_engine.py use, so all
#    engines produce output structure_analysis.py / pdf_assembly.py can
#    consume identically ────────────────────────────────────────────────────
...
class OpenRouterOCREngine(OCREngine):
    """
    OCR engine backed by OpenRouter's OpenAI-compatible API.
    ...
    def __init__(self, config: dict):
        self.config      = config
        # OPENROUTER_MODEL (env) wins over ocr.openrouter_model_name
        # (config.yaml). These are the fallback defaults used when the
        # Settings page hasn't stored a user-provided value in Redis (see
        # refresh_runtime_settings). Resolve to "" if neither is set — the
        # first OCR call then fails fast with an actionable error rather
        # than guessing a model slug.
        self._default_model   = openrouter_model(config.get("openrouter_model_name"))
        self._default_api_key = os.environ.get("OPENROUTER_API_KEY", "").strip()
        ...
        # No universal rate-limit ceiling to clamp to here (unlike Poe's
        # flat 500/min) — OpenRouter's limits are mostly inherited from
        # whichever upstream provider serves the chosen model. Fall back to
        # a conservative default (matching OpenRouter's documented
        # free-model ceiling) only when rpm_limit is entirely absent from
        # config; otherwise use it as-is — tune it in config.yaml to match
        # your model (free vs. paid) if you hit 429s.
        self.rpm_limit = max(1, int(config.get("rpm_limit", 20)))
    ...
    def load(self) -> None:
        ...
        # the user may supply OPENROUTER_API_KEY / OPENROUTER_MODEL later via
        # the app's Settings page (stored in Redis, applied by
        # refresh_runtime_settings at the start of each job) rather than at
        # worker-boot time. Failing here would permanently kill the worker
        # thread before the user ever gets a chance to configure anything
        # through the UI. The first actual OCR call raises a clear,
        # actionable error instead if neither source has provided
        # credentials by then.
        if not self.api_key or not self.model_name:
            logger.warning(
                "OpenRouter engine loaded without OPENROUTER_API_KEY/OPENROUTER_MODEL "
                "configured. Set them on the app's Settings page, or via "
                "OPENROUTER_API_KEY / OPENROUTER_MODEL environment variables, "
                "before starting a job."
            )
        import httpx
        logger.info(f"Initialising OpenRouter client (model={self.model_name or '(not set)'}, rpm={self.rpm_limit})…")
        self._client = httpx.Client(base_url=OPENROUTER_API_BASE, timeout=self.timeout_s)
        self._loaded = True
        logger.info("OpenRouter client ready.")

    def refresh_runtime_settings(self, r) -> None:
        """
        Re-resolve api_key / model_name from Redis
        (settings:openrouter_api_key / settings:openrouter_model), which the
        Settings page writes via POST /api/settings. A value stored there
        takes priority over the OPENROUTER_API_KEY / OPENROUTER_MODEL
        environment variable (or ocr.openrouter_model_name in config.yaml)
        this engine was constructed with; if Redis has nothing stored (or
        the user cleared it), that original default is used instead. Called
        once per job by worker.py — not part of the OCREngine ABC, so
        callers should use hasattr() before calling it.
        """
        try:
            redis_key   = (r.get("settings:openrouter_api_key") or "").strip()
            redis_model = (r.get("settings:openrouter_model") or "").strip()
        except Exception as e:
            logger.warning(f"Could not read OpenRouter settings from Redis: {e}")
            return
    ...
    def _analyse_page(self, page_image) -> dict:
        """
        Run one OpenRouter API call for this page, cache the result, and
        return the parsed dict {"direction": ..., "blocks": [...]}.

        Reusing the cached result across detect_direction / recognize /
        get_layout means each PDF page costs exactly ONE OpenRouter request.
        """
        self._assert_loaded()
        ...
        result = self._call_openrouter_with_retry(jpeg_bytes)
        ...
        if not non_empty_blocks:
            ...
            logger.info(
                f"OpenRouter returned 0 text blocks for this page (image {w}×{h}px). "
    ...
    def _call_openrouter_with_retry(self, jpeg_bytes: bytes) -> dict:
        import httpx

        if not self.api_key or not self.model_name:
            raise RuntimeError(
                "OpenRouter OCR is not configured — set your OpenRouter API key "
                "and model slug on the app's Settings page (or the "
                "OPENROUTER_API_KEY / OPENROUTER_MODEL environment variables) "
                "before starting a job."
            )
        ...
        headers = {
            ...
            # Optional attribution headers OpenRouter's docs recommend for
            # its public rankings — harmless to include, and never required
            # for the request itself to succeed.
            "HTTP-Referer": "https://github.com/sophia-chingyan/OCR-image-based-pdf_accept-jpg-upload",
            "X-Title": "OCR for Imaged PDF",
        }
        ...
        while attempt < max_attempts:
            ...
            try:
                ...
                logger.warning(f"OpenRouter call failed: {e} — retry {attempt}/{max_attempts} in {wait:.1f}s")
                time.sleep(wait)
                continue
            ...
            if response.status_code >= 400:
                ...
                if response.status_code in RETRYABLE_CODES and attempt < max_attempts:
                    ...
                    logger.warning(
                        f"OpenRouter API transient error (HTTP {response.status_code}) — "
            ...
            try:
                ...
                logger.warning(f"Unexpected OpenRouter response shape: {str(data)[:300]}")
                text = ""
            return self._parse_response(text)

        code_str = f"HTTP {last_code} " if last_code else ""
        raise RuntimeError(
            f"OpenRouter API failed after {attempt} attempts ({code_str}— last error: {last_exc})"
        )
        ...
        try:
            ...
            if start >= 0 and end > start:
                try:
                    ...
                    logger.warning("Could not parse OpenRouter response as JSON.")
                    return {"direction": "horizontal", "blocks": []}
            else:
                logger.warning("OpenRouter response did not contain JSON.")
                return {"direction": "horizontal", "blocks": []}
    ...
    def _assert_loaded(self):
        if not self._loaded:
            raise RuntimeError("OpenRouterOCREngine.load() must be called before use.")
